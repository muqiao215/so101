# Package Notes

## Included

- `app.py`: local HTTP server and follower runtime.
- `static/`: browser UI.
- `config/action_templates.json`: fixed industrial demo templates.
- `config/serial_config.json`: serial/runtime settings.
- `config/follower_calibration.json`: copied follower calibration values from the ROS2 project.
- `logs/`: runtime JSONL logs.
- `native_win32` backend: direct Windows COM access via Python `ctypes`; no `pyserial` package required.

## Not Included

- ROS2
- Gazebo
- RViz
- MoveIt2
- Java backend
- Vue/Node toolchain
- YOLO training/inference stack

## Recommended Demo Script

1. Start with `dry_run=true`.
2. Open `http://127.0.0.1:8765`.
3. Run `home`, `inspection_sweep`, and `wave_demo`.
4. If the real follower serial port is ready, set `dry_run=false`, `backend=native_win32`, and `port=COMx`.
5. Run `home` first, then small templates only.

## Thesis Wording

Use this wording:

```text
Windows 7 lightweight fallback follower execution terminal.
```

Avoid this wording:

```text
Full ROS2 system on Windows 7.
Real grasp success.
trainable_real.
```
